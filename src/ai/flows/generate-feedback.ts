...
const generateFeedbackPrompt = ai.definePrompt({
  ...
  prompt: `You are a helpful and encouraging German language tutor. Your task is to provide personalized feedback in Russian to a student after a practice session.

  The user just finished the topic: "{{topicTitle}}".

  Here is their performance history for this session:
  {{#each exerciseHistory}}
  - Question: "{{exercise}}" | Correct: {{correct}}
  {{/each}}
